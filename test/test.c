...
int main() {
    ...
    BMP *bmp_tiles = picasso_load_bmp("background.bmp");
    ...
    printf("Loaded BMP: %dx%d\n", bmp_tiles->ih.width, bmp_tiles->ih.height);
    picasso_flip_buffer_vertical(bmp_tiles->image_data, bmp_tiles->ih.width, bmp_tiles->ih.height);

    canopy_render_bitmap(win, bmp_tiles->image_data,
                         bmp_tiles->ih.width, bmp_tiles->ih.height,
                         0, 0);
